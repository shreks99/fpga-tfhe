...
int main(int argc, char** argv) {
    ...
    if (posix_memalign((void**) &out_mu, 4096, sizeof(Torus32_FPGA))) throw std::bad_alloc();
    if (posix_memalign((void**) &out_x, 4096, sizeof(LweSample_FPGA))) throw std::bad_alloc();
    if (posix_memalign((void**) &out_bk, 4096, sizeof(LweBootstrappingKeyFFT_FPGA))) throw std::bad_alloc();
    /*====================================================TFHE CLOUD===============================================================*/
    ...
    //std::cout << "MU: " << MU << " b: " << temp_result->b << std::endl;
    //std::cout << "AMIN: " << bk->bkFFT->in_out_params->alpha_min << " AMAX: " << bk->bkFFT->in_out_params->alpha_max << std::endl;
    *in_mu = MU; 
    //memcpy(in_mu, &MU, sizeof(Torus32_FPGA));
    ...
    /* OUTPUT BUFFERS */
    OCL_CHECK(err, cl::Buffer buffer_out_bk(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, sizeof(LweBootstrappingKeyFFT_FPGA), out_bk, &err)); 
    ...
    OCL_CHECK(err, err = krnl2.setArg(8, buffer_out_bk));
    OCL_CHECK(err, err = krnl2.setArg(9, buffer_out_mu));
    OCL_CHECK(err, err = krnl2.setArg(10, buffer_out_x));
    ...
    if (*out_mu != MU) {
        std::cout << "out_mu: " << *out_mu << " MU " << MU << std::endl; 
        match = false; 
    }
    
    // match = verifyLweparams(&(in_bk->in_out_params), &(out_bk->in_out_params));
    // match = verifyTlweparams(&(in_bk->accum_params), &(out_bk->accum_params));
    // match = verifyTgswparams(&(in_bk->bk_params), &(out_bk->bk_params)); 
    // match = verifyLweparams(&(in_bk->extract_params), &(out_bk->extract_params));
