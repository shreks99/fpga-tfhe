...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl4 = cl::Kernel(program, "mulByXaiOrCopy_return", &err));
            valid_device = true;
    ...
    *in_mu = MU; 
    ...
    OCL_CHECK(err, err = krnl4.setArg(2, buffer_out_testvect));
    OCL_CHECK(err, err = krnl4.setArg(3, buffer_out_bara));
    ...
    for (int i = 0; i < N; i++) {
        if (v->coefsT[i] != out_testvect[i]) {
            std::cout << i << ": v: " << v->coefsT[i] << " out_testvect " << out_testvect[i] << std::endl; 
            match = false; 
        }
    }
