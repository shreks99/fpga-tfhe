...
//     libOutputFile.close();
// }
void convertTLweSampleToTLweSampleFPGA(const TLweSample* src, TLweSample_FPGA* dest, int32_t k, int32_t N) {
    // Copy each TorusPolynomial in 'a' to TorusPolynomial_FPGA in 'a'
    for (int i = 0; i <= k; ++i) {
        for (int j = 0; j < N; ++j) {
            dest->a[i].coefsT[j] = src->a[i].coefsT[j];
        }
    }

    // Copy the TorusPolynomial in 'b' to TorusPolynomial_FPGA in 'b'
    for (int j = 0; j < N; ++j) {
        dest->b.coefsT[j] = src->b->coefsT[j];
    }

    // Copy the current_variance
    dest->current_variance = src->current_variance;
}
...
int main(int argc, char** argv) {
    ...
    if (posix_memalign((void**) &accum_FPGA, 4096, sizeof(TLweSample_FPGA))) throw std::bad_alloc();
    ...
    *in_mu = MU; 
    ...
    // Perform the conversion for FPGA parameters
    Torus32_FPGA bara_FPGA[n];
    convertTLweSampleToTLweSampleFPGA(acc, accum_FPGA, accum_params->k, accum_params->N);
    for (int32_t i = 0; i < n; i++) bara_FPGA[i] = bara[i];
 ...
 //   OCL_CHECK(err, cl::Buffer buffer_in_mu(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(Torus32_FPGA), in_mu, &err)); 
  //  OCL_CHECK(err, cl::Buffer buffer_in_x(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(LweSample_FPGA), in_x, &err)); 
    OCL_CHECK(err, cl::Buffer buffer_accum_FPGA(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(TLweSample_FPGA), accum_FPGA, &err)); 
   OCL_CHECK(err, cl::Buffer buffer_in_bara_FPGA(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(Torus32_FPGA)* n, bara_FPGA, &err)); 
    ...
    OCL_CHECK(err, err = krnl4.setArg(0, buffer_accum_FPGA));
    OCL_CHECK(err, err = krnl4.setArg(1, buffer_in_bara_FPGA));
    ...
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_accum_FPGA}, 0 /* 0 means from host*/));
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_in_bara_FPGA}, 0 /* 0 means from host*/));
    q.finish();
    htod = clock() - htod; 
